...
These points already enable TW to be used as an offline-enabled single web page application.
Also, by using a server side node.js environment, TW can be used as an online web application, just by providing an additional plugin to persist tiddlers into plain text files and a plugin syncing the local data store with the node.js server.
